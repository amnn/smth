...
[![CI](https://github.com/amnn/smth/actions/workflows/ci.yml/badge.svg)](https://github.com/amnn/smth/actions/workflows/ci.yml)
...
Bind `smth` to tmux's session-switcher key, and move tmux's built-in session
chooser to `C-b S`:
...
The `-d "#{pane_current_path}"` option runs `smth` from the pane that was active
...
named jj workspace, `smth` uses the recorded default workspace as the repository
...
You can also pass one or more repository globs to `smth`. Command-line globs
stack with `repo.globs` from config:

```tmux
bind s display-popup -E -w 80% -h 80% -T smth -d "#{pane_current_path}" \
  "smth -r ~/Code/'*' -r ~/.bootstrap -r ~/.config/nvim"
```
...
`smth` is then configured to call it as follows:
...
`smth` reads configuration from `$XDG_CONFIG_HOME/smth/smth.toml`, or
`~/.config/smth/smth.toml` when `$XDG_CONFIG_HOME` is unset. You can also pass
