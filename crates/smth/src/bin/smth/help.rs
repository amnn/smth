...
//! Help output formatting for the `smth` CLI.
...
fn write_config_help<W: Write>(w: &mut Writer<W>) -> io::Result<()> {
    ...
    writeln!(w)?;
    w.paragraph(
        "The config file is TOML. It is optional; when no file is present, smth uses the \
         built-in defaults.",
    )?;
    ...
    w.code(|out| {
        ...
        writeln!(out, "    \"-group\", \"smth:{{pane}}\",")?;
        writeln!(out, "    \"-execute\", [")?;
        writeln!(out, "      \"/Users/me/.config/smth/focus-pane\",")?;
